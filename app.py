  ...
  <div class="row">
    <div class="left">
      ...
      <div class="menu">
        ...
        <a href="#map">About ▾</a>
        <a href="https://github.com/BlazeVeritas-AI/blazeveritas-project" target="_blank">Docs ▾</a>
      </div>
    </div>
  </div>
</div>
...
with tabs[0]:
    ...
    with c2:
        ...
        st.markdown(
            '<div class="callout">🚀 Faster, explainable triage reduces false dispatches and time-to-decision.</div>',
            unsafe_allow_html=True,
        )
...
"""
    ...
    # -------------------------------------------------------------------
    # About section – anchor used by top nav: href="#map"
    # -------------------------------------------------------------------
    st.markdown(
        """
<a id="map"></a>
...
This project is part of the *Master of Science in Software Engineering* program at **IT Park University**, developed with support from **EPAM Engineering School**.

Its purpose is to build an AI-powered system for wildfire detection and monitoring using visual imagery.  
The system integrates deep-learning models, Grad-CAM explainability, uncertainty calibration, and a RAG-based Copilot to support early detection, transparent reasoning, and operational decision-making.

### 🔭 Future Work
- Deployment to **Microsoft Azure**  
- Integration with **real-time satellite data** (NASA FIRMS, Sentinel-2)  
- Expansion of the RAG knowledge base with climate and emergency datasets  
- Optimization for edge-device inference  
- Automated alerting & geospatial analytics  
""",
        unsafe_allow_html=True,
    )
        ...
        if up is not None:
            ...
            if st.button("Run Detection (Upload)", use_container_width=True):
                ...
                if r and r.ok:
                    d = r.json()
                    st.success(
                        f"**{d['label'].upper()}** | p={d['prob']:.3f} | "
                        f"H={d['uncertainty']:.3f} • event_id: {d['event_id']}"
                    )
...
with tabs[3]:
    st.markdown("<a id='copilot'></a>", unsafe_allow_html=True)
    ...
    def _send_to_copilot(objective: str):
        """Minimal call to FastAPI /v1/copilot/plan endpoint with defaults."""
        payload = dict(
            lat=None,
            lon=None,
            label="unknown",
    ...
    st.markdown(
        "<div class='cp-title'>Hello! 🌤 — What wildfire insight can I help with today?</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='cp-sub'>RAG-grounded answers and structured action plans — in a clean, chat-first UI.</div>",
        unsafe_allow_html=True,
    )
    ...
    with colS:
        ...
        with c1:
            if st.button("🛰 Analyze image", use_container_width=True):
                prompt = (
                    "Analyze the latest detection image if available. "
        ...
        with c2:
            if st.button("📊 Summarize report", use_container_width=True):
                prompt = (
                    "Summarize key risks and mitigation strategies for the current region based on retrieved docs. "
        ...
        with c3:
            if st.button("🧭 Generate plan", use_container_width=True):
                prompt = (
                    "Generate an action plan with: Situation summary, confidence & uncertainties, key risks "
        ...
        with c4:
            if st.button("💡 Ask weather impact", use_container_width=True):
                prompt = (
                    "How will wind direction and humidity affect potential fire spread in the next 6 hours? "
    ...
    with chat_box:
        ...
        with st.container(border=True):
            if not st.session_state.copilot_msgs:
                st.markdown(
                    "<div class='cp-hint'>No messages yet. Use a quick action above, or ask a question below.</div>",
                    unsafe_allow_html=True,
                )
            else:
...
# -------------------------------------------------------------------
# Map tab (placeholder for future geospatial UI)
# -------------------------------------------------------------------
